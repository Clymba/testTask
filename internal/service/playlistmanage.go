...
func (s *Service) GetSongByID(ctx context.Context, id uuid.UUID) (*repository.Song, error) {
	if id == uuid.Nil {
		return nil, errors.New("не тот id")
	}
...
func (s *Service) GetFilteredSongs(ctx context.Context, groupName, text, genre, link string, page, limit int) ([]repository.Song, error) {
	logger.Log.Infof("Received filter parameters: groupName=%s, text=%s, genre=%s, page=%d, limit=%d", groupName, text, genre, page, limit)

	// Получаем песни из репозитория
	songs, err := s.repository.GetFilteredSongs(ctx, groupName, text, genre, link, page, limit)
	if err != nil {
	...
	return songs, nil // Возвращаем найденные песни
}

func (s *Service) UpdateSong(ctx context.Context, song *repository.Song) error {
	if song == nil {
		return errors.New("пустая песня")
	}
